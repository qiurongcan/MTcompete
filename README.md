...
1. 小车位置分为两类，一类接机区，一类等待上货区域
2. 其中，1号4号小车接机和等待上货为同一位置
```python
AGV-SIM1: [187, 431, -16]
AGV-SIM4: [193, 431, -16]
```
3. 2号3号小车共用一个等待上货区，接机区不同，同理5号6号小车共用一个等待上货区，接机区不同
```python
# 等待上货区
AGV-SIM2&3: [181,431, -16]
# 接机区域
AGV-SIM2: [186, 438, -16]
AGV-SIM3: [181, 440. -16]

# 等待上货区
AGV-SIM5&6: [199, 431, -16]
# 接机区域
AGV-SIM5: [194, 438, -16]
AGV-SIM6: [199, 440, -16]
```
4. 小车从等待上货区出发到上货点
5. 小车需要避开其他小车（3m），避免碰撞
6. 小车上货后离开上货点，前往接机点起飞。如果是2，3号、5、6号小车，他们在到达等待上货区停一下，然后让飞机起飞，再前往接机区，让飞机早一些起飞。
7. 使用一个car_state变量来判断各个小车的状态。0-初始化状态，前往上货点，上货后变为1；1-小车上有飞机，飞机上有货物，可以起飞，起飞后变为2；2-等待飞机返回，飞机返回则前往上货点，状态变为1
8. 执行完每一次循环发布一个话题`/car_state`，用于对补充调度节点的控制
...
用于控制2号&3号小车， 5号&6号小车 是否可以前往等待上货区域
根据`/car_state`话题进行判断
例如，2号小车首先需要判断自身状态是否可以前往目的地，然后再判断等待上货区是否可以前往。
...
## 10.27 能够优化的地方
1. 飞机电量低的时候，不进行充电，使用更换电池，初步估算能够节约3min时间，可以提高0-600分
2. 小车前往目的地的顺序

## 10.28可能存在BUG
1.在线上测试的时候会出现碰撞，有时候也不会出现碰撞
2.目前没找到问题所在


## 10.29 分支说明
1. final_v7 是稳定版本
2. final_v8 是稳定版本+不充电采用更换飞机的版本
3. final_v6 存在一些BUG，不适用

## 10.30 后续可以优化的方向
1. 根据当前时刻的订单情况，动态调整小车运行顺序，尽可能的获取高分
2. 一个目的地设置往返两条航线。但是不一定能够接到单，要求速度要尽可能快
